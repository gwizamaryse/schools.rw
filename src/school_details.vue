...
<v-card  flat tile align-content-center>
    <v-card-text class="display-2">
        <p > {{SCHOOL.school_1.school_name}}  </p>
...
<!-- Basic Information -->
  <v-flex d-flex>
            ...
            <v-btn large color="error"> {{SCHOOL.school_1.school_type[0]}} </v-btn>
            <v-btn  large color="success"> {{SCHOOL.school_1.school_type[1]}} </v-btn>
            <v-btn large color="warning"> {{SCHOOL.school_1.school_type[2]}} </v-btn>
            <v-btn large class="info"> {{SCHOOL.school_1.school_type[3]}} </v-btn>   
  ...
  <v-layout row wrap >
    ...
    <p class="title"> {{SCHOOL.school_1.basic_info.number_students}} </p>
    </v-flex>
      <v-flex xs4 sm3 text-xs-center>
    <v-icon size="80" color="light-blue"> supervised_user_circle</v-icon>
    <p class="title"> {{SCHOOL.school_1.basic_info.students_per_class}} </p>
    </v-flex>
    <v-flex xs4 sm3 text-xs-center>
    <v-icon size="80" color="light-blue"> language </v-icon>
    <p class="title"> {{SCHOOL.school_1.basic_info.languages[0]}} <br> {{SCHOOL.school_1.basic_info.languages[1]}} <br> {{SCHOOL.school_1.basic_info.languages[2]}} </p>
    </v-flex>
    <v-flex xs4 sm3 text-xs-center>
    <v-icon size="80" color="light-blue">fas fa-tshirt</v-icon>
    <p class="title">{{SCHOOL.school_1.basic_info.uniform}}</p>
    </v-flex>
    <v-flex xs4 sm3 text-xs-center>
...
export default {
  data () {
    return {
      ...
      tab: null,
    items: [
      'Basic Info', 'Fees', 'Ranking', 'Admissions', 'Curriculum'
    ],
    SCHOOL:{},
  ...
  methods: {
    getSchoolData : function(){
      const _this=this;
      axios.get('/lib/json/school_details.json')
      .then(function(res){
